...
<body>
  <nav class=" navbar navbar-expand navbar-light navbar-light" style="background-color: #E98B33;">
    <a class="navbar-brand" href="#" style="color: white; font-weight: bold; font-size: 1.5em;">
      <img src="<?= base_url('assets/dist/img/LOGO_BKN.png')?>" alt="Logo" width="50" height="60"
        class="d-inline-block align-top"
        style="box-shadow: 0 4px 12px rgba(239,126,27,0.5); border-radius: 10px; background: #fff; padding: 4px;">
    </a>
    <ul class="navbar-nav">
...
$_POST['tanggal']
  ...
  // require_once 'config/koneksi.php'; // Sudah di-require di atas
  ...
  $sql = "INSERT INTO kritik_saran (nama_lengkap, instansi, jabatan, kritik, saran, penilaian, kontak, tanggal)
                  VALUES ('$nama_lengkap', '$instansi', '$jabatan', '$kritik', '$saran', '$penilaian', '$kontak', '$tanggal')";

  if ($koneksi->query($sql)) {
    echo "<script>alert('Kritik & Saran berhasil dikirim!'); window.location = 'index.php';</script>";
  }
  else {
    $error_message = $koneksi->error;
    echo "<script>alert('Gagal mengirim kritik & saran. Error: $error_message');</script>";
  }
}
?>

      <!-- Modal Kritik & Saran -->
      <div class="modal fade" id="kritikSaranModal" tabindex="-1" role="dialog" aria-labelledby="kritikSaranModalLabel"
        aria-hidden="true">
        ...
        <div id="fotoSlider" class="carousel slide col-12" data-ride="carousel">
          ...
          <div class="carousel-inner">
            <div class="carousel-item active">
              <img src="<?= base_url('assets/dist/img/WEB_ZI_BARU.jpg')?>" class="d-block w-100 h-100" alt="Slider 1"
                style="height:350px;object-fit:cover;">
            </div>
            <div class="carousel-item">
              <img src="<?= base_url('assets/dist/img/WEB1_Kantor.jpg')?>" class="d-block w-100 h-100" alt="Slider 2"
                style="height:350px;object-fit:cover;">
            </div>
          </div>
                ...
                <div class="form-row align-items-end">
                  <div class="form-group col-md-5">
                    ...
                    <input type="text" class="form-control" name="nip" placeholder="Masukkan NIP"
                      value="<?= isset($_GET['nip']) ? htmlspecialchars($_GET['nip']) : ''?>">
                  </div>
                  <div class="form-group col-md-5">
                    <label for="id_konsultasi">ID Konsultasi</label>
                    <input type="text" class="form-control" name="id_konsultasi" placeholder="Masukkan ID Konsultasi"
                      value="<?= isset($_GET['id_konsultasi']) ? htmlspecialchars($_GET['id_konsultasi']) : ''?>">
                  </div>
                  <div class="form-group col-md-2">
                    <button type="submit" class="btn btn-success btn-xl" style="margin-top: 32px;">
                      <i class="fas fa-search"></i>
                    </button>
                    <button type="button" class="btn btn-secondary btn-xl" style="margin-top: 32px; margin-left: 5px;"
              ...
              <div class="alert alert-dark">
                ...
                <table class="table table-bordered mt-2">
                  <tr>
                    <th>ID Konsultasi</th>
                    <td>
                      <?= htmlspecialchars($data['id_konsultasi'])?>
                    </td>
                  </tr>
                  <tr>
                    <th>NIP</th>
                  ...
                  <tr>
                    ...
                    <td>
                      <?= htmlspecialchars($data['tanggal_respon'])?>
                    </td>
                  </tr>
                </table>
                <button type="button" class="btn btn-primary" data-toggle="modal" data-target="#modalFilterCetakRespon"
                  data-nip="<?= urlencode($nip)?>" data-id="<?= urlencode($id_konsultasi)?>">
