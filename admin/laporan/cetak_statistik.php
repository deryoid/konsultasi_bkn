...
if (!empty($tanggal_awal) && !empty($tanggal_akhir)) {
    ...
    $sql_filter_date = " AND k.tanggal_pengajuan BETWEEN '$tanggal_awal_safe' AND '$tanggal_akhir_safe'";
...
$stats = $koneksi->query("
    ...
    WHERE 1=1 $sql_filter_date
    GROUP BY c.id_kategori
    ORDER BY jumlah DESC
");

$total_konsultasi = $koneksi->query("SELECT COUNT(*) FROM konsultasi k WHERE 1=1 $sql_filter_date")->fetch_row()[0];
$total_menunggu = $koneksi->query("SELECT COUNT(*) FROM konsultasi k WHERE k.status = 'Menunggu' $sql_filter_date")->fetch_row()[0];
$total_diproses = $koneksi->query("SELECT COUNT(*) FROM konsultasi k WHERE k.status = 'Diproses' $sql_filter_date")->fetch_row()[0];
$total_selesai = $koneksi->query("SELECT COUNT(*) FROM konsultasi k WHERE k.status = 'Selesai' $sql_filter_date")->fetch_row()[0];
...
while ($row = $stats_result->fetch_assoc()) {
    ...
    $list_judul = $koneksi->query("
        ...
        FROM konsultasi k
        WHERE k.id_kategori = '$id_kategori' $sql_filter_date
...
<body>
    <div class="header">
        <img src="<?php echo base_url('assets/dist/img/LOGO_BKN.png')?>" alt="Logo BKN"
            style="width: 60px; height: 75px; margin-bottom: 10px;">
...
?>
            <tr>
                ...
                <td class="text-center">
                    <?php echo $row['menunggu']?>
                </td>
                <td class="text-center">
                    <?php echo $row['diproses']?>
                </td>
                <td class="text-center">
                ...
                <td>
                    <div style="background: #eee; border-radius: 3px; height: 20px; position: relative;">
                        <div
                            style="background: #667eea; height: 100%; border-radius: 3px; width: <?php echo $persentase?>%;">
                        </div>
                        <span style="position: absolute; top: 2px; left: 5px; font-size: 10px;">
                            <?php echo $persentase?>%
                        </span>
                    </div>
                </td>
...
endwhile; ?>
    ...
    <h2 style="margin: 30px 0 20px 0; font-size: 16px; border-bottom: 2px solid #667eea; padding-bottom: 10px;">
        Rangkuman Judul Konsultasi per Kategori</h2>
    ...
    <div class="kategori-section">
        <div class="kategori-title">
            ...
            <?php echo count($kategori['detail'])?> konsultasi)
        </div>
        <table>
            <thead>
                <tr>
...
?>
                <tr>
                    ...
                    <td>
                        <?php echo $judul['tanggal']?>
                    </td>
                    <td>
                        <?php echo htmlspecialchars($judul['judul'])?>
                    </td>
                    <td><span class="badge <?php echo $badge_class?>">
                            <?php echo $judul['status']?>
                        </span></td>
                </tr>
                <?php
        endforeach; ?>
...
endforeach; ?>
    ...
    <!-- Grafik Sederhana -->
    <h2 style="margin: 30px 0 20px 0; font-size: 16px; border-bottom: 2px solid #667eea; padding-bottom: 10px;">Grafik
        Distribusi Kategori</h2>
    <table>
        <?php
$stats->data_seek(0);
$max_jumlah = 0;
        ...
        <tr>
            ...
            <td width="50%">
                <div style="background: #eee; border-radius: 3px; height: 25px; position: relative;">
                    <div
                        style="background: #667eea; height: 100%; border-radius: 3px; width: <?php echo $bar_width; ?>%;">
                    </div>
                </div>
            </td>
...
endwhile; ?>
    ...
    <div class="footer">
        ...
        <p>&copy;
            <?php echo date('Y')?> Badan Kepegawaian Negara. All rights reserved.
        </p>
